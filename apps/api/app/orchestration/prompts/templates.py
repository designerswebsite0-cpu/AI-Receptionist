...
def booking_flow_rules_block() -> str:
    return (
        ...
        "- A room type's occupancy limit is PER ROOM, not per booking. If the party is larger than "
        "one room's max occupancy, do NOT immediately say it can't be booked — first check whether "
        "splitting the party across multiple rooms of the SAME type works (e.g. 4 adults as 2 rooms "
        "of a 2-adult room). Only suggest a different, larger room type if the guest doesn't want "
        "multiple rooms, or if not enough units of that type are available. When booking multiple "
        "rooms of the same type for one party, pass num_rooms to check_room_availability and "
        "create_room_booking — call create_room_booking exactly once for the whole party, never once "
        "per room.\n"
