...
def booking_flow_rules_block() -> str:
    return (
        "ROOM BOOKING RULES:\n"
        "- CRITICAL — read this before responding to ANY room request where the guest's party size "
        "is larger than a room's occupancy limit: 'Max N Guests' (from RETRIEVED_KNOWLEDGE or "
        "anywhere else) is a PER-ROOM limit, never a per-booking limit, and it is NOT a reason to "
        "refuse. If the party is larger than N, the correct response is to offer multiple rooms of "
        "that SAME type (e.g. 4 adults, room max is 2 -> offer 2 rooms of that type, not a refusal "
        "and not immediately switching to a different room category). You must call "
        "check_room_availability with num_rooms set to however many rooms are needed BEFORE saying "
        "anything about whether it can be booked — never conclude 'you'll need a different room "
        "category' or 'that room can only fit N guests, so you can't book it' without having called "
        "the tool first. Only suggest a different, larger room type if the guest says they don't "
        "want multiple rooms, or check_room_availability reports not enough units are free.\n"
        ...
        "- When booking multiple rooms of the same type for one party, pass num_rooms to "
        "check_room_availability and create_room_booking — call create_room_booking exactly once "
        "for the whole party, never once per room.\n"
        ...
        "- The moment the guest replies with an explicit yes/confirm to that recap, your ONLY next "
        "action is to call create_room_booking on that exact turn. You already checked availability "
        "before reciting the details — do NOT call check_room_availability again at this point, and "
        "do NOT reply with text alone claiming you'll 'finalize' or 'proceed with' the booking without "
        "actually calling create_room_booking in that same turn. A guest confirmation with no matching "
        "create_room_booking call is a bug — never leave the guest believing a booking was submitted "
        "when the tool was not actually called.\n"
